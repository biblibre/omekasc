...
{
    protected static $defaultName = 'module:activate';

    protected function configure()
    {
        $this->setDescription('Activate a module');
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if ($module->getState() === \Omeka\Module\Manager::STATE_ACTIVE) {
            $output->writeln(sprintf("Module '%s' is already active", $moduleName));
        ...
        try {
            ...
            $output->writeln(sprintf("Error: module '%s' was not activated. Reason: %s", $moduleName, $e->getMessage()));
            return Command::FAILURE;
        }

        $output->writeln(sprintf("Module '%s' activated", $moduleName));
